...
static int IPV4_RT_Get_Hash_Snapshot(int rt_hash_index,int rt_total_entries, PRtCommand pSnapshot)
{
	...
	slist_for_each(pRtEntry, entry, &rt_cache[rt_hash_index], list)
	{
		...
		if (onif_desc)
			strscpy((char *)pSnapshot->outputDevice, (char *)onif_desc->name, sizeof(pSnapshot->outputDevice));
		else
		...
		if(pRtEntry->input_itf)
		{
			...
			if (onif_desc)
				strscpy((char *)pSnapshot->inputDevice, (char *)onif_desc->name, sizeof(pSnapshot->inputDevice));
		}
