...
int cmmFeL2FlowUpdate(FCI_CLIENT* fci_handler, int request, struct l2flowTable *l2flow_entry)
{
	...
	case FPP_ACTION_REGISTER:
		...
		ret = fci_write(fci_handler, FPP_CMD_RX_L2FLOW_ENTRY, sizeof(fpp_l2_bridge_flow_entry_cmd_t), &cmd);
...
int __cmmFeRouteUpdate(FCI_CLIENT* fci_handler, int action, struct fpp_rt *fpp_route)
{
	...
	case FPP_ACTION_REGISTER:
		...
		ret = fci_write(fci_handler, FPP_CMD_IP_ROUTE, sizeof(fpp_rt_cmd_t), &cmd);
	...
	*res_len = 2;
	...
	if(cmd->enable)
	{
		if(!globalConf.ff_enable)
		{
			...
			ret = fci_cmd(fci_handler, FPP_CMD_IPV4_FF_CONTROL, cmd, sizeof(fpp_ff_ctrl_cmd_t), res_buf, res_len);
