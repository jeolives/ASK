...
int cmmFeBridgedIfUpdate(FCI_CLIENT *fci_handle, int fd, struct interface *itf)
{
	...
	if (fci_write(fci_handle, FPP_CMD_BRIDGED_ITF_UPDATE, sizeof(br_cmd), &br_cmd)){
...
int cmmBridgeInit(struct cmm_ct *ctx)
{
	...
	if (fci_write(ctx->fci_handle, FPP_CMD_RX_L2BRIDGE_MODE, sizeof(br_cmd), &br_cmd)){
...
int cmmRxQueryProcess(char ** keywords, int tabStart, daemon_handle_t daemon_handle)
{
	...
	{
		int count = 0;
		/* Buffer size includes room for ".<svlan>.<cvlan>" suffix
		 * appended below; IFNAMSIZ alone overflows on max-length
		 * names from the FPP daemon IPC channel. */
